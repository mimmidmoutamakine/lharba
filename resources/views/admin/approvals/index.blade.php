    ...
    <div class="space-y-8" dir="rtl">
        <div class="flex items-center justify-between gap-4">
            <div class="text-right">
                ...
                <p class="mt-2 text-sm text-slate-500">الطلبات الجديدة ديال الامتحان والمستوى.</p>
            </div>
            <div class="rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-700">
                المعلقة الآن: {{ $pendingRequests->count() }}
            </div>
        </div>
                    ...
                    @foreach ($pendingRequests as $pendingRequest)
                        <div class="grid gap-4 px-6 py-5 lg:grid-cols-[1fr_1.3fr]">
                            <div class="grid gap-3 sm:grid-cols-2">
                                <form method="POST" action="{{ route('admin.approvals.approve', $pendingRequest) }}" class="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-right">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('admin.approvals.reject', $pendingRequest) }}" class="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-right">
                                    @csrf
                            ...
                            <div class="text-right">
                                <h3 class="text-lg font-semibold text-slate-900">{{ $pendingRequest->user->name }}</h3>
                                <p class="mt-1 text-sm text-slate-500">{{ $pendingRequest->user->email }}</p>
                                <p class="mt-3 text-sm text-slate-700">الامتحان: <strong>{{ $pendingRequest->examFamily->name }}</strong></p>
                                <p class="mt-1 text-sm text-slate-700">المستوى: <strong>{{ $pendingRequest->level }}</strong></p>
                                <p class="mt-1 text-sm text-slate-700">الهدف: <strong>{{ $pendingRequest->study_goal }}</strong></p>
                                <p class="mt-1 text-sm text-slate-700">الوقت اليومي: <strong>{{ $pendingRequest->daily_minutes }} دقيقة</strong></p>
                                <p class="mt-3 text-sm text-slate-600">
                                    تسجّل في {{ optional($pendingRequest->submitted_at)->format('Y-m-d H:i') }}
                                </p>
                            </div>
                        </div>
                    @endforeach
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm" dir="rtl">
                        <thead class="bg-slate-50 text-slate-600">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right font-medium">الامتحان</th>
                                <th class="px-6 py-3 text-right font-medium">المستوى</th>
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @foreach ($recentRequests as $recentRequest)
                                <tr>
                                    <td class="px-6 py-4 font-medium text-slate-900">{{ $recentRequest->user->name }}</td>
                                    <td class="px-6 py-4 text-slate-600">{{ $recentRequest->examFamily->name }}</td>
                                    <td class="px-6 py-4 text-slate-600">{{ $recentRequest->level }}</td>
                                    <td class="px-6 py-4">
                                        <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $recentRequest->status === 'approved' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">
                                            {{ $recentRequest->status === 'approved' ? 'مقبول' : 'مرفوض' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-slate-600">{{ $recentRequest->review_note ?: '—' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
